...
void interrupt_handler(int interrupt, struct interupt_frame* frame) {
  outb(0x20, 0x20);
}
...
void idt_set(int interupt_no, void* addr) {
  struct idt_desc* desc = &idt_descriptors[interupt_no];
...
void idt_init() {
  memset(idt_descriptors, 0, sizeof(PEACHOS_TOTAL_INTERRUPTS));
...
void isr80h_register_command(int command_id, ISR80H_COMMAND command) {
  if (command_id < 0 || command_id >= PEACHOS_MAX_ISR80H_COMMANDS) {
    panic("isr80h_register_command: commdand_id out of bounds.\n");
  }

  if (isr80h_commands[command_id]) {
    panic("isr80h_register_command: commdand_id already taken.\n");
  }
