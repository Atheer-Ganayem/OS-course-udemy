...
void task_save_state(struct task* task, struct interupt_frame* frame) {
...
void task_current_save_state(struct interupt_frame* frame) {
  ...
  if (!task) {
    panic("task_current_save_frame: NO current task to save!\n");
  }
...
void task_run_first_ever_task() {
  if (!current_task) {
    panic("task_run_first_ever_task(): No current tasks exist!");
  }
...
int task_init(struct task* task, struct process* proc) {
  ...
  if (proc->filetype == PROCESS_FILE_TYPE_ELF) {
