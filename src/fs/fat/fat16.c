...
typedef unsigned int FAT_ITEM_TYPE; // internal-use thingies
...
struct fat_directory_item {
  ...
  uint16_t  low_16_bits_fist_cluster;
  uint32_t  file_size;
} __attribute__((packed));
...
int fat16_get_total_items_for_directory(struct disk* disk, int dir_start_sector) {
  struct fat_directory_item item;
...
int fat16_get_root_directory(struct disk* disk, struct fat_private* private) {
  struct fat_directory* directory = &private->root_directory;
  int res = 0;
  ...
  if (diskstreamer_seek(stream, root_dir_sector_pos * disk->sector_size) != PEACHOS_ALL_OK) {
    res = -EIO;
    goto out;
  }
...
int fat16_resolve(struct disk* disk) {
  ...
  if (fat16_get_root_directory(disk, fat_private) != PEACHOS_ALL_OK) {
    res = -EIO;
    goto out;
  }
...
void fat16_to_propper_string(char **out, const char *in) {
...
void fat16_get_full_relative_filename(struct fat_directory_item* item, char* out, int max_len) {
  ...
  char *out_tmp = out;
  fat16_to_propper_string(&out_tmp, (const char*) item->filename);
  if (item->ext[0] != 0x00 && item->ext[0] != 0x20) {
    *out_tmp++ = '.';
    fat16_to_propper_string(&out_tmp, (const char*) item->ext);
  }
}
...
static uint32_t fat16_get_first_cluster(struct fat_directory_item* item) {
  return (item->high_16_bits_first_cluster << 16) | item->low_16_bits_fist_cluster;
}
...
static int fat16_get_fat_entry(struct disk* disk, int cluster) {
  ...
  res = diskstreamer_seek(stream, fat_table_pos * cluster * PEACHOS_FAT16_FAT_ENTRY_SIZE);
  if (res < 0) {
    goto out;
  }
...
static int fat_16_read_internal_from_stream(struct disk* disk, struct disk_stream* stream, int cluster, int offset, int total, void* out) {
  int res = 0;
  ...
  if (total > 0) {
    ...
    res = fat_16_read_internal_from_stream(disk, stream, cluster, offset+total_to_read, total, out+total_to_read);
  }
...
static int fat16_read_internal(struct disk* disk, int starting_cluster, int offset, int total, void* out) {
  ...
  return fat_16_read_internal_from_stream(disk, stream, starting_cluster, offset, total, out);
}

void fat_16_free_directory(struct fat_directory* directory) {
  if (!directory) return;
...
void fat16_fat_item_free(struct fat_item* item) {
  if (item->type == FAT_ITEM_TYPE_DIRECTORY) {
    fat_16_free_directory(item->directory);
...
struct fat_directory* fat16_load_fat_directory(struct disk* disk, struct fat_directory_item* item) {
  ...
  int total_item = fat16_get_total_items_for_directory(disk, cluster_sector);
...
out:
  if (res != PEACHOS_ALL_OK) {
    fat_16_free_directory(directory);
  }
  return directory;
}
...
int fat16_stat(struct disk* disk, void* private, struct file_stat* stat) {
  ...
  struct fat_directory_item* ritem = fat_desc->item->item;
